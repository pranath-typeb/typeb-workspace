...
import { updateProject, type BillingType, type Project, type ProjectStatus } from '../data/projects'
...
export default function EditProjectModal({ project, onClose }: EditProjectModalProps) {
  ...
  const [billing, setBilling] = useState<BillingType>(project.billing)
  const [billable, setBillable] = useState(project.billable)
  const [staffing, setStaffing] = useState(project.staffing)
  const [starts, setStarts] = useState(project.starts)
  const [ends, setEnds] = useState(project.ends)
  const [managerId, setManagerId] = useState(project.managerId ?? '')
  const [teamIds, setTeamIds] = useState<string[]>(project.teamIds)
  const [keywords, setKeywords] = useState<string[]>(project.calendarKeywords ?? [])
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" style={{ width: 560, maxHeight: 'calc(100vh - 64px)', overflowY: 'auto' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div className="serif" style={{ fontSize: 20, letterSpacing: '-0.6px' }}>Edit Project</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 4 }}>
          ...
          <button className="btn-dark" disabled={!canSubmit} onClick={submit}>Save Changes</button>
        </div>
      </div>
    </div>
  )
